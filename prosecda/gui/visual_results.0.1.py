...
#        self.canvasframe.draw_architecture(architecture=self.current_architecture_instance())
        if not self.cb_var.get():
            ...
            # draws architectures
            self.canvasframe.draw_architecture(architecture=selected_arch_instance)
        else:
    ...
    def show_architectures(self):
        if self.cb_var.get():
            ...
            # draws architectures
            self.canvasframe.draw_architectures(protein=selected_prot_instance)
        else:
            self.canvasframe.canvas.delete()
            
            # draws currently selected architecture
            self.canvasframe.draw_architecture(architecture=self.current_architecture_instance())   
            self.canvasframe.canvas.update()
...
class Canvasframe(tk.Frame):
    ...
    def create_rectangle(self, x0, y0, width, height, name='', **kwargs):
        ...
        if 'alpha' in kwargs:
            ...
            self.canvas.create_image(x0, y0, image=self.images[-1], anchor='nw')
        self.canvas.create_rectangle(x0, y0, x0+width, y0+height, **kwargs)

        self.canvas.create_text((x0 + (x0 + width)) / 2.,
                                                 (y0 + (y0 + height)) / 2.,
                                                 text=name,
                                                 font=('Arial', 9, 'bold'),
                                                 fill='white',
    ...
    def draw_sequence(self, x0, y0, width):
        self.canvas.create_line(x0, y0, width, y0, width=1)
...
#                              fill='blue', stipple="gray50")
        self.create_rectangle(x0=x0, y0=y0, width=width, height=height,
                              name=scaled_domain['name'], fill='blue', alpha=.7)
    ...
    def draw_architecture(self, architecture):
        ...
        x0, y0 = 1, 50
        coef = 500.
        self.draw_sequence(x0=x0, y0=y0, width=coef)
        
        scaled_domains = architecture.scale_domains(coef=coef)
